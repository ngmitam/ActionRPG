	...
	const FGameplayEventData *TriggerEventData)
{
	...
	if(BuffEffectClasses.Num() > 0 && ActorInfo->OwnerActor.IsValid())
	{
		for(const TSubclassOf<UGameplayEffect> &Effect : BuffEffectClasses)
		{
			if(Effect)
			{
				FActiveGameplayEffectHandle BuffHandleTemp =
					ApplyGameplayEffectToOwner(Handle, ActorInfo,
						ActivationInfo, Effect.GetDefaultObject(),
...
void UMyGameplayAbility::EndAbility(const FGameplayAbilitySpecHandle Handle,
	const FGameplayAbilityActorInfo *ActorInfo,
	const FGameplayAbilityActivationInfo ActivationInfo,
