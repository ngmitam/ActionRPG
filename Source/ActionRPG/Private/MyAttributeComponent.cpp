...
UMyAttributeComponent::UMyAttributeComponent() {
  ...
  // timing issues They will be set in DeferredInitialize after AbilityActorInfo
  // is initialized
...
void UMyAttributeComponent::DeferredInitialize() {
  ...
  if (AttributeSet && !AbilitySystemComponent->HasAttributeSetForAttribute(
  ...
  if (AbilitySystemComponent->AbilityActorInfo.IsValid()) {
    ...
    // Still not ready, try again next frame (with a limit to prevent infinite
    // loops)
...
void UMyAttributeComponent::InitializeAttributes() {
  ...
  SetDefaultAttributes(DefaultHealth, DefaultMaxHealth, DefaultStamina,
                       DefaultMaxStamina);
}
...
void UMyAttributeComponent::OnStaminaChange(
    const FOnAttributeChangeData &Data) {
  if (Data.NewValue <= 0.0f && AbilitySystemComponent &&
    ...
    SetSprinting(false);
    // Note: The actual ability cancellation should be handled by the ability
    // system or character
  }
}
...
void UMyAttributeComponent::SetDefaultAttributes(float Health, float MaxHealth,
                                                 float Stamina,
                                                 float MaxStamina) {
  if (AbilitySystemComponent &&
