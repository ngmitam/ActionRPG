...
void UMyAttackAbility::EndAbility(
    const FGameplayAbilitySpecHandle Handle,
    const FGameplayAbilityActorInfo *ActorInfo,
    const FGameplayAbilityActivationInfo ActivationInfo,
...
void UMyAttackAbility::EndComboWindow() { bInComboWindow = false; }
  ...
  if (DamageEffectToUse) {
    ...
    if (Target) {
      ...
      if (DamageSpecHandle.IsValid()) {
        // Get damage amount from the effect's default object
        UMyDamageEffect *DefaultEffect =
            Cast<UMyDamageEffect>(DamageEffectToUse.GetDefaultObject());
        if (DefaultEffect) {
          float DamageAmount = DefaultEffect->BaseDamage;
          // Set the damage magnitude (negative to subtract from health)
        ...
        TargetData->HitResult = FHitResult(Target, nullptr, FVector::ZeroVector,
                                           FVector::ZeroVector);
...
AActor *UMyAttackAbility::FindTargetInRange() {
  ...
  GetWorld()->OverlapMultiByChannel(OverlapResults, Location, FQuat::Identity,
                                    ECC_Pawn, Sphere);
...
void UMyAttackAbility::OnMontageEnded(UAnimMontage *Montage,
                                      bool bInterrupted) {
  if (!bInterrupted) {
    if (bComboQueued) {
      PlayAttackMontage();
